...
import { MapPin, Phone, Mail, Send, CheckCircle, X, AlertTriangle } from 'lucide-react';
...
const ContactPage = () => {
  ...
  const provinces = [
    ...
    'Cao Bằng', 'Cần Thơ', 'Đà Nẵng', 'Đắk Lắk', 'Đắk Nông', 'Đồng Nai',
    'Đồng Tháp', 'Điện Biên', 'Gia Lai', 'Hà Giang', 'Hà Nam', 'Hà Nội',
    'Hà Tĩnh', 'Hải Dương', 'Hải Phòng', 'Hoà Bình', 'Hậu Giang', 'Hưng Yên',
    'TP. Hồ Chí Minh', 'Khánh Hòa', 'Kiên Giang', 'Kon Tum', 'Lai Châu', 'Lào Cai',
    'Lạng Sơn', 'Lâm Đồng', 'Long An', 'Nam Định', 'Nghệ An', 'Ninh Bình',
  ...
  const handleSubmit = () => {
    if (!validateForm()) {
      showAlert('error', 'Lỗi xác thực', 'Vui lòng kiểm tra lại thông tin đã nhập');
      return;
    }

    console.log('Form data:', formData);
    
    showAlert('success', 'Thành công!', 'Cảm ơn bạn đã đăng ký! Chúng tôi sẽ liên hệ với bạn sớm nhất.');
    resetForm();
  };
  ...
  const AlertComponent = ({ alert, onClose }) => {
    ...
    return (
      <div className={`fixed top-4 right-4 z-50 max-w-sm w-full ${style.bg} border rounded-lg shadow-lg p-4 transition-all duration-300 animate-in slide-in-from-top-2`}>
        <div className="flex items-start">
          <IconComponent className={`${style.iconColor} flex-shrink-0 mr-3 mt-0.5`} size={20} />
          <div className="flex-1">
            <h3 className={`font-semibold ${style.titleColor} text-sm`}>{alert.title}</h3>
    ...
    <div className="min-h-screen bg-gradient-to-br from-orange-50 to-blue-50">
      ...
      <div className="relative h-90 overflow-hidden">
        ...
        <div className="absolute inset-0 flex flex-col items-center justify-center text-white text-center px-4">
          <h1 className="text-4xl md:text-5xl font-bold mb-4 drop-shadow-lg">LIÊN HỆ</h1>
        </div>
      </div>
        ...
        <div className="max-w-2xl mx-auto">
          <div className="bg-white rounded-xl shadow-lg p-8">
            <div className="text-center mb-8">
              <h2 className="text-3xl font-bold text-gray-800 mb-4">Đăng ký tư vấn</h2>
              <div className="w-20 h-1 bg-gradient-to-r from-orange-500 to-orange-600 mx-auto rounded-full"></div>
              <p className="text-gray-600 mt-4">Để lại thông tin để được tư vấn miễn phí</p>
            </div>
